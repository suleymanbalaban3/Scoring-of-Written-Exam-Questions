...
public class Sentence{
	...
	public void genereteSecondMethodAverageVector() {
		averageWordVector = new ArrayList<>(200);
		...
		for (int i = 0; i < getWordOfSentence().size(); i++) {
			double average = genereteAverageOneVector(getWordOfSentence().get(i));
			for (int j = 0; j < getWordOfSentence().get(i).getWordDenseVector().size(); j++) {
				getAverageWordVector().set(j, getAverageWordVector().get(j) + ( (average * getWordOfSentence().get(i).getWordDenseVector().get(j) ) / getWordOfSentence().size()));
			}
		}
	}
	public double genereteAverageOneVector(Word word) {
		double average = 0.0;

	
			
		for (int i = 0; i < word.getWordDenseVector().size(); i++) {
			average += word.getWordDenseVector().get(i);
		}
		return average / word.getWordDenseVector().size();
	}
}
